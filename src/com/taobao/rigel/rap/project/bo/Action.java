...
public class Action implements java.io.Serializable {
	...
	public String getTable() {
		return getParameterListHTML(requestParameterList, responseParameterList);
	}


	private String getParameterListHTML( Set<Parameter> listRequest, Set<Parameter> listResponse ) {
		StringBuilder html = new StringBuilder();
		html.append("<table class=\"param-table\" align='center' valign='middle' >");
		getOtherInformation(html);
		getTh(html);
		getParameterListHTMLSub(html, listRequest, (short) 1);
		getParameterListHTMLSubResponse(html, listResponse, (short) 1);
		html.append("</table>");
	...
	/*	private String getParameterListHTML(Set<Parameter> list) {
			...
			.append("<thead>")
	        .append("<th class=\"th-name\">字段名称</th>")
	        .append("<th class=\"th-identifier\">字段类型</th>")
	        .append("<th class=\"th-type\">是否为空</th>")
	...
	private String getOtherInformation( StringBuilder html ) {
		html.append("<tr><td>接口名称</td><td colspan='3'>" + name + "</td></tr>")
				.append("<tr><td>接口描述</td><td colspan='3'>" + description + "</td></tr>")
				.append("<tr><td colspan='4'></td></tr>").append("<tr><td>请求类型</td><td colspan='3'>form</td></tr>")
				.append("<tr><td>请求方式</td><td colspan='3'>" + getMethod() + "</td></tr>")
				.append("<tr><td>请求地址</td><td colspan='3'>" + requestUrl + "</td></tr>");
		return html.toString();
	}


	private String getTh( StringBuilder html ) {
		html
				.append(
					"<tr style='height:20px;background:#ff99cc;color:#000000;font-size:12px;font-weight:bold;text-align:center;border:1px solid #000;'>")
				.append("<td class=\"th-name\">字段名称</th>").append("<td class=\"th-identifier\">字段类型</th>")
				.append("<td class=\"th-type\">是否为空</th>").append("<td class=\"th-remark\">备注</th>").append("</tr>");
		return html.toString();
	}
	...
	private void getParameterListHTMLSub( StringBuilder html, Set<Parameter> list, short level ) {
		for ( Parameter p : list ) {
			html
					...
					.append(
						"<td class=\"td-name\">"
								+ levelMark(level) + StringUtils.escapeInH(p.getIdentifier()) + "</td>")
					.append("<td class=\"td-identifier\">" + StringUtils.escapeInH(p.getDataType()) + "</td>")
					.append("<td class=\"td-type\">" + StringUtils.escapeInH(p.getRemarkWithoutMockjsRule()) + "</td>")
					.append("<td class=\"td-remark\">" + StringUtils.escapeInH(p.getName()) + "</td>").append("</tr>");
	...
	private String getJsonData( Set<Parameter> list ) {
		JSONObject json = new JSONObject();
		for ( Parameter p : list ) {
			if ( p.getDataType().contains("array") ) {
				if ( p.getParameterList() != null && p.getParameterList().size() > 0 ) {
					json.put(p.getIdentifier(), "[" + getJsonData(p.getParameterList()) + "]");
				} else {
					json.put(p.getIdentifier(), "[" + p.getName() + "]");
				}
			} else {
				if ( p.getParameterList() != null && p.getParameterList().size() > 0 ) {
					json.put(p.getIdentifier(), getJsonData(p.getParameterList()));
	...
	private void getParameterListHTMLSubResponse( StringBuilder html, Set<Parameter> list, short level ) {
		JsonFormatTool tool = new JsonFormatTool();
		html.append("<tr><td colspan='4'></td></tr>").append(
			"<tr><td style='word-break:keep-all;white-space:nowrap;'>应答报文格式</td><td colspan='3' style='text-align:left'>"
					+ tool.formatJson(getJsonData(list)) + "</td></tr>");
		getTh(html);
		for ( Parameter p : list ) {
			html
					.append("<tr class=\"tr-level-" + level + "\">")
	...
	public String getRequestUrlRel() {
		...
		if ( url == null || url.isEmpty() ) { return "xxxxxxxxxxxxxxxxEMPTY URLxxxxxxxxxxxxxxxxx"; }
